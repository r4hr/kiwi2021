...
kiwi21_original %>% 
  ...
  summarise(cant = n()) %>% 
  ungroup %>% 
  ggplot(aes(y = Trabajo, x = cant)) +
  geom_col(fill = azul) +
  estilov +
  geom_text(aes(label = cant), # Indica la cantidad de decimales
...
Si comparamos los resultados con la [edición del 2020](https://rpubs.com/Data4HR/encuesta-kiwi-2020) (sección *Sueldos Argentina -> Distribución de sueldos por puesto*) podemos apreciar las siguientes diferencias. 
...
Para realizar este análisis tuvimos que unificar varias respuestas y eliminar algunas que no correspondían al área de RRHH para poder quedarnos con estas áreas de análisis.
...
rh_ar <- rh_ar %>% 
  mutate(funcion = str_trim(funcion, side = "both"),
    funcion = fct_collapse(funcion, 
                           "Adm. de personal" = c("Administración de personal", 
                                                  "Administrativa en área",
                                                  "Asistente"),
                           "Generalista" = c("Generalista", 
                                             "Al ser HRBP un poco de todo payroll, capacitacion, comunicacion interna,",
                                             "Area integral de RH", 
                                             "General",
                                             "HRBP",
                                             "La mayoría de los nombrados con iguales condiciones de importancia y prioridad",
                                             "RRHH", 
                                             "Todas las anteriores", 
                                             "Todo", 
                                             "Todo lo anterior", 
                                             "Todos ellos", 
                                             "Todos, los descriptos"),
...
rh_ar %>% 
  group_by(funcion) %>% 
  filter(funcion != "Otras funciones") %>% 
  summarise(sueldo_promedio = mean(sueldo_ft)) %>% 
  ggplot(aes(y = fct_reorder(funcion, sueldo_promedio), 
             x = sueldo_promedio)) +
  geom_col(fill = azul) +
  labs(title = "Sueldo promedio por función",
       subtitle = "En AR$",
...
rh_ar %>% 
  filter(funcion != "Otras funciones") %>% 
  group_by(funcion, puesto) %>% 
  summarise(sueldo_medio = mean(sueldo_ft)) %>% 
  ggplot(aes(x = funcion, y = sueldo_medio)) +
  geom_col(fill = azul) +
  eje_y_n +
  estiloh +
  facet_wrap(~puesto) +
...
```{r gap_funciones}
funciones_top <- rh_ar %>% 
  filter(funcion %in% c("People Analytics", "Diseño organizacional",
                           "Relaciones laborales", "Compensaciones y beneficios")) %>% 
  summarise(promedio = mean(sueldo_ft)) %>% 
  pull()

funciones_bottom <- rh_ar %>% 
  filter(funcion %in% c("Payroll", "Reclutamiento y selección", 
